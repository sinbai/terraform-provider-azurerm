...
func TestAccComputeFleet_virtualMachineProfileNetwork_update(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		data.ImportStep(
			...
			"additional_location_profile.0.virtual_machine_profile_override.0.os_profile.0.linux_configuration.0.admin_password"),
		{
			Config: r.netWorkProfileCompleteUpdate(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r ComputeFleetTestResource) netWorkProfileComplete(data acceptance.TestData) string {
	return fmt.Sprintf(`
%[1]s
...
`, r.baseAndAdditionalLocationLinuxTemplate(data), r.netWorkProfileResourceDependencies(data), data.RandomInteger, data.Locations.Primary, data.Locations.Secondary)
}

func (r ComputeFleetTestResource) netWorkProfileCompleteUpdate(data acceptance.TestData) string {
	return fmt.Sprintf(`
%[1]s
...
resource "azurerm_application_security_group" "linux_test-other" {
  location            = azurerm_resource_group.linux_test.location
  name                = "TestApplicationSecurityGroupOther"
  resource_group_name = azurerm_resource_group.linux_test.name

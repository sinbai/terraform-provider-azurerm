...
func (r MongoClusterResource) Arguments() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"create_mode": {
			...
			// 'Default' and `GeoReplica` are the only accepted value currently, other values will be supported later.
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			case string(mongoclusters.CreateModeDefault):
				if _, ok := metadata.ResourceDiff.GetOk("administrator_login"); !ok {
					return fmt.Errorf("`administrator_login` is required when `create_mode` is specified as %s", string(mongoclusters.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("administrator_login_password"); !ok {
					return fmt.Errorf("`administrator_login_password` is required when `create_mode` is specified as  %s", string(mongoclusters.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("compute_tier"); !ok {
					return fmt.Errorf("`compute_tier` is required when `create_mode` is specified as %s", string(mongoclusters.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("storage_size_in_gb"); !ok {
					return fmt.Errorf("`storage_size_in_gb` is required when `create_mode` is specified as  %s", string(mongoclusters.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("high_availability_mode"); !ok {
					return fmt.Errorf("`high_availability_mode` is required when `create_mode` is specified as  %s", string(mongoclusters.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("shard_count"); !ok {
					return fmt.Errorf("`shard_count` is required when `create_mode` is specified as  %s", string(mongoclusters.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("version"); !ok {
					return fmt.Errorf("`version` is required when `create_mode` is specified as  %s", string(mongoclusters.CreateModeDefault))
				}

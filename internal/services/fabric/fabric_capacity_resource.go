...
func (r FabricCapacityResource) ModelObject() interface{} {
	return &FabricCapacityResourceModel{}
}
...
func (r FabricCapacityResource) IDValidationFunc() pluginsdk.SchemaValidateFunc {
	return fabriccapacities.ValidateMongoClusterID
}
...
func (r FabricCapacityResource) Arguments() map[string]*pluginsdk.Schema {
	return map[string]*pluginsdk.Schema{
		...
		"create_mode": {
			...
			Optional: true,
			ForceNew: true,
			Default:  string(fabriccapacities.CreateModeDefault),
			ValidateFunc: validation.StringInSlice([]string{
				string(fabriccapacities.CreateModeDefault),
				string(fabriccapacities.CreateModeGeoReplica),
			}, false),
		},
		...
		"source_server_id": {
			...
			Optional:     true,
			ForceNew:     true,
			ValidateFunc: fabriccapacities.ValidateMongoClusterID,
		},
		...
		"preview_features": {
			...
			Elem: &pluginsdk.Schema{
				Type:         pluginsdk.TypeString,
				ValidateFunc: validation.StringInSlice(fabriccapacities.PossibleValuesForPreviewFeature(), false),
			},
		},
		...
		"high_availability_mode": {
			...
			Optional: true,
			ValidateFunc: validation.StringInSlice([]string{
				string(fabriccapacities.HighAvailabilityModeDisabled),
				string(fabriccapacities.HighAvailabilityModeZoneRedundantPreferred),
			}, false),
		},
...
func (r FabricCapacityResource) Create() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			id := fabriccapacities.NewMongoClusterID(subscriptionId, state.ResourceGroupName, state.Name)
			...
			parameter := fabriccapacities.MongoCluster{
				Location:   azure.NormalizeLocation(state.Location),
				Name:       pointer.To(state.Name),
				Properties: &fabriccapacities.MongoClusterProperties{},
			}

			if _, ok := metadata.ResourceData.GetOk("administrator_login"); ok {
				parameter.Properties.Administrator = &fabriccapacities.AdministratorProperties{
			...
			if _, ok := metadata.ResourceData.GetOk("compute_tier"); ok {
				parameter.Properties.Compute = &fabriccapacities.ComputeProperties{
			...
			if _, ok := metadata.ResourceData.GetOk("shard_count"); ok {
				parameter.Properties.Sharding = &fabriccapacities.ShardingProperties{
					ShardCount: pointer.To(state.ShardCount),
				}
			}

			if _, ok := metadata.ResourceData.GetOk("storage_size_in_gb"); ok {
				parameter.Properties.Storage = &fabriccapacities.StorageProperties{
					SizeGb: pointer.To(state.StorageSizeInGb),
				}
			}

			if _, ok := metadata.ResourceData.GetOk("create_mode"); ok {
				parameter.Properties.CreateMode = pointer.To(fabriccapacities.CreateMode(state.CreateMode))
			}

			if _, ok := metadata.ResourceData.GetOk("high_availability_mode"); ok {
				parameter.Properties.HighAvailability = &fabriccapacities.HighAvailabilityProperties{
					TargetMode: pointer.To(fabriccapacities.HighAvailabilityMode(state.HighAvailabilityMode)),
				}
			}

			if state.PublicNetworkAccessEnabled {
				parameter.Properties.PublicNetworkAccess = pointer.To(fabriccapacities.PublicNetworkAccessEnabled)
			} else {
				parameter.Properties.PublicNetworkAccess = pointer.To(fabriccapacities.PublicNetworkAccessDisabled)
			}

			if state.CreateMode == string(fabriccapacities.CreateModeGeoReplica) {
				parameter.Properties.ReplicaParameters = &fabriccapacities.MongoClusterReplicaParameters{
...
func (r FabricCapacityResource) Update() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			id, err := fabriccapacities.ParseMongoClusterID(metadata.ResourceData.Id())
			if err != nil {
				return err
			}
			...
			if metadata.ResourceData.HasChange("compute_tier") {
				model.Properties.Compute = &fabriccapacities.ComputeProperties{
					Tier: pointer.To(state.ComputeTier),
				}
			...
			if metadata.ResourceData.HasChange("administrator_login_password") {
				model.Properties.Administrator = &fabriccapacities.AdministratorProperties{
					UserName: pointer.To(state.AdministratorLogin),
					Password: pointer.To(state.AdministratorLoginPassword),
				}
			}
			...
			if metadata.ResourceData.HasChange("high_availability_mode") {
				model.Properties.HighAvailability = &fabriccapacities.HighAvailabilityProperties{
					TargetMode: pointer.To(fabriccapacities.HighAvailabilityMode(state.HighAvailabilityMode)),
				}
			}
			...
			if metadata.ResourceData.HasChange("public_network_access_enabled") {
				if state.PublicNetworkAccessEnabled {
					model.Properties.PublicNetworkAccess = pointer.To(fabriccapacities.PublicNetworkAccessEnabled)
				} else {
					model.Properties.PublicNetworkAccess = pointer.To(fabriccapacities.PublicNetworkAccessDisabled)
				}
			}

			if metadata.ResourceData.HasChange("storage_size_in_gb") {
				model.Properties.Storage = &fabriccapacities.StorageProperties{
					SizeGb: pointer.To(state.StorageSizeInGb),
				}
			}

			if metadata.ResourceData.HasChange("shard_count") {
				model.Properties.Sharding = &fabriccapacities.ShardingProperties{
					ShardCount: pointer.To(state.ShardCount),
				}
			}
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if model := resp.Model; model != nil {
				...
				if props := model.Properties; props != nil {
					...
					state.PublicNetworkAccessEnabled = pointer.From(props.PublicNetworkAccess) == fabriccapacities.PublicNetworkAccessEnabled
...
func (r FabricCapacityResource) CustomizeDiff() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		...
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			...
			if state.CreateMode == string(fabriccapacities.CreateModeDefault) || state.CreateMode == "" {
				if _, ok := metadata.ResourceDiff.GetOk("administrator_login"); !ok {
					return fmt.Errorf("`administrator_login` is required when `create_mode` is not specified or is specified as %s", string(fabriccapacities.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("administrator_login_password"); !ok {
					return fmt.Errorf("`administrator_login_password` is required when `create_mode` is not specified or is specified as  %s", string(fabriccapacities.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("compute_tier"); !ok {
					return fmt.Errorf("`compute_tier` is required when `create_mode` is not specified or is specified as %s", string(fabriccapacities.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("storage_size_in_gb"); !ok {
					return fmt.Errorf("`storage_size_in_gb` is required when `create_mode` is not specified or is specified as  %s", string(fabriccapacities.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("high_availability_mode"); !ok {
					return fmt.Errorf("`high_availability_mode` is required when `create_mode` is not specified or is specified as  %s", string(fabriccapacities.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("shard_count"); !ok {
					return fmt.Errorf("`shard_count` is required when `create_mode` is not specified or is specified as  %s", string(fabriccapacities.CreateModeDefault))
				}

				if _, ok := metadata.ResourceDiff.GetOk("version"); !ok {
					return fmt.Errorf("`version` is required when `create_mode` is not specified or is specified as  %s", string(fabriccapacities.CreateModeDefault))
				}
			}

			if state.ComputeTier == "Free" || state.ComputeTier == "M25" {
				if state.HighAvailabilityMode == string(fabriccapacities.HighAvailabilityModeZoneRedundantPreferred) {
...
func expandPreviewFeatures(input []string) *[]fabriccapacities.PreviewFeature {
	result := make([]fabriccapacities.PreviewFeature, 0)

	for _, v := range input {
		if v != "" {
			result = append(result, fabriccapacities.PreviewFeature(v))
		}
	}

...
func resourceMsSqlDatabaseUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChange("transparent_data_encryption_key_vault_key_id") {
		if v := d.Get("transparent_data_encryption_key_vault_key_id").(string); v != "" {
			keyId, err := keyVaultParser.ParseNestedItemID(v)
			if err != nil {
				return fmt.Errorf("unable to parse key: %q: %+v", v, err)
			}

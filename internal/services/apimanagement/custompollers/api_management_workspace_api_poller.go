...
var (
	worksapceApiPollingSuccess = pollers.PollResult{
		Status:       pollers.PollingStatusSucceeded,
		PollInterval: 10 * time.Second,
	}
	worksapceApiPollingInProgress = pollers.PollResult{
		Status:       pollers.PollingStatusInProgress,
		PollInterval: 10 * time.Second,
	}
)

// NewAPIManagementWorkspaceAPIPoller - creates a new poller for API Management API operations to handle the case there is a query string
...
func (p apiManagementWorkspaceAPIPoller) Poll(ctx context.Context) (*pollers.PollResult, error) {
	if p.asyncID == "" {
		return &worksapceApiPollingSuccess, nil
	}
	...
	case http.StatusAccepted, http.StatusCreated:
		return &worksapceApiPollingInProgress, nil
	}

	return nil, fmt.Errorf("unexpected status code %d", resp.StatusCode)
}

// NewAPIManagementWorkspaceApiDeletePoller - creates a new poller for API Management long-running delete operation to handle the case where the delete operation is asynchronous.

...
var _ sdk.ResourceWithUpdate = ApiManagementGatewayConfigConnectionResource{}
...
func (r ApiManagementGatewayConfigConnectionResource) Update() sdk.ResourceFunc {
	return sdk.ResourceFunc{
		Timeout: 30 * time.Minute,
		Func: func(ctx context.Context, metadata sdk.ResourceMetaData) error {
			client := metadata.Client.ApiManagement.ApiGatewayConfigConnectionClient
			...
			if metadata.ResourceData.HasChange("hostnames") {
				properties.Properties.Hostnames = pointer.To(model.Hostnames)
			}

			if err := client.CreateOrUpdateThenPoll(ctx, *id, pointer.From(properties)); err != nil {

...
~> **NOTE:** When the `sku_name` is `DW100c`, the `transparent_data_encryption_key_automatic_rotation_enabled` and the `transparent_data_encryption_key_vault_key_id` could not be specified as database-level CMK is not supported for Data Warehouse SKUs.

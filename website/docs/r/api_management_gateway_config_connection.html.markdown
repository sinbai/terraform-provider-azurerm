...
description: |-
  Manages an API Management Gateway Config Connection.
---
...
* `name` - (Required) The name of the Gateway Config Connection. Changing this forces a new resource to be created.
...
* `source_id` - (Required) The ID of the source that this Gateway Config Connection will connect to. Changing this forces a new resource to be created.

* `hostnames` - (Optional) A list of hostnames for this Gateway Config Connection.
...
* `update` - (Defaults to 30 minutes) Used when updating the API Management Gateway Config Connection.
